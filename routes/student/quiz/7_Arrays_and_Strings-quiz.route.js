...
router.get('/', async (req, res, next) => {
  ...
  MongoClient.connect(url, function(err, db) {
    ...
    dbo.collection("StudentAnswer").find(query).toArray(function(err, StudentAnswer) {
      if (err) throw err;
      if(Object.keys(StudentAnswer).length < 15){
        res.redirect('/pretest7_check')
      }
      else{
        MongoClient.connect(url, function(err, db) {
          if (err) throw err;
          ...
          dbo.collection("StudentRecommendation").find(query).toArray(function(err, RecommendaResult) {
            ...
            res.render('student/quiz/7_Arrays_and_Strings-quiz', { person ,StudentAnswer,RecommendaResult,ADRI1,ADRI2,ADRI3,ADRI4,ADRI5});
          });
        });
      }
      db.close();
...
router.post('/submit', async (req, res, next) => {
  ...
  var currentQuiz = "Arrays_and_Strings-(Post-test)" //*** */
  var timetodo = 0;
