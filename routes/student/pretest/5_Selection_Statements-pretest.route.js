      ...
      if(Object.keys(StudentAnswer).length === 0){
        ...
        MongoClient.connect(url, function(err, db) {
          ...
          dbo.collection("StudentRecommendation").find(query).toArray(function(err, RecommendaResult) {
            ...
            res.render('student/pretest/5_Selection_Statements-pretest', { person ,StudentAnswer,RecommendaResult,ADRI1,ADRI2,ADRI3,ADRI4,ADRI5});
          });
        });
      }
      db.close();
...
router.post('/submit', async (req, res, next) => {
  ...
  var currentQuiz = "Selection_Statements-(Pre-test)" //*** */
  var timetodo = 0;
...
if(choice15 === 'C'){
  ...
  try {
      ...
      res.redirect('/course-Selection_Statements')    
  } catch (error) {
    next(error);
  }
});
